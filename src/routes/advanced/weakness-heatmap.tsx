...
function WeaknessHeatmap() {
  ...
  const [detail, setDetail] = useState<{ topic: string; items: Record<string, unknown>[] } | null>(
    null,
  );
  const refresh = useApiMutation(
    () => api("/api/advanced/heatmap/refresh", { method: "POST" }),
    ["heatmap"],
  );
  ...
  return (
    <AppShell>
      <PageHeader
        title="Viva Weakness Heatmap"
        subtitle="Your weak topics, aggregated across every viva session you've taken."
      />
      ...
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-12">
        <Card className="lg:col-span-7">
          <h3 className="flex items-center gap-2 text-base font-semibold">
            <Grid3X3 className="h-4 w-4" /> Topics (weakest first)
          </h3>
          ...
          <div className="mt-4 grid grid-cols-2 gap-2 md:grid-cols-3">
            {(heatmap ?? []).map((h) => (
              <button
                key={h.topic}
                onClick={() => openDetail(h.topic)}
                className={`rounded-lg p-3 text-left text-sm ${cellColor(h.avg_score)}`}
              >
                <div className="font-semibold capitalize">{h.topic}</div>
                <div className="mt-1 flex items-center justify-between text-xs">
                  <span>
                    {h.avg_score}/100 · {h.question_count} Qs
                  </span>
          ...
          {!isLoading && !(heatmap ?? []).length && (
            <p className="mt-4 text-sm text-muted-foreground">
              No data yet — complete a few AI viva sessions first.
            </p>
          )}
        </Card>
        <Card className="lg:col-span-5">
          ...
          {detail ? (
              ...
              {detail.items.map((q, i) => (
                <div key={i} className="rounded-lg border p-2 text-xs">
                  ...
                  <p className="mt-1 text-muted-foreground">
                    Score: {String(q.score)} — {String(q.feedback ?? "")}
                  </p>
                </div>
              ))}
            </div>
            ...
            <p className="mt-3 text-sm text-muted-foreground">
              Click a weak topic to see the full question history and feedback.
            </p>
          )}
        </Card>
      </div>

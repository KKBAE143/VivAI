  ...
  return (
    <AppShell fitViewport hideTopBar>
      <div className="flex flex-col gap-2.5 lg:gap-3 h-full lg:overflow-hidden overflow-y-auto font-manrope">
        {/* Compact Integrated Apple Header */}
        <div className="flex shrink-0 items-center justify-between gap-3 apple-glass-card rounded-[22px] px-3.5 sm:px-4 py-2.5">
          <div className="flex items-center gap-3">
            <Link
              to="/ai-viva"
              className="grid h-9 w-9 shrink-0 place-items-center rounded-xl border border-border bg-black/5 dark:bg-white/5 text-muted-foreground hover:text-foreground hover:bg-black/10 dark:hover:bg-white/10 active:scale-95 transition-all no-underline"
          ...
          <div className="flex items-center gap-2">
            <Link
              to="/ai-viva"
              className="apple-glass-btn-secondary inline-flex min-h-[34px] items-center justify-center px-3 py-1.5 text-xs font-semibold no-underline"
            >
              Cancel
            </Link>
            <button
              ...
              className="apple-glass-btn-primary inline-flex min-h-[34px] items-center justify-center gap-1.5 px-4 py-1.5 text-xs font-bold uppercase tracking-wider cursor-pointer disabled:cursor-not-allowed disabled:opacity-40"
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-2.5 lg:gap-3 flex-1 min-h-0">
          ...
          <div className="lg:col-span-7 flex flex-col gap-2.5 lg:gap-3 min-h-0 justify-between">
                    ...
                    <button
                      ...
                      className={`rounded-xl border p-2.5 text-left transition-all cursor-pointer active:scale-[0.98] ${
                        active
                          ...
                          : "border-border bg-black/5 dark:bg-white/5 hover:border-primary/40 text-foreground"
                      }`}
                    >
            ...
            <div className="apple-glass-card p-3.5 flex-1 flex flex-col justify-center">
              ...
              {isProjectViva ? (
                <div>
                  ...
                  >
                    <SelectTrigger className="w-full min-h-[42px] rounded-xl border border-border bg-card px-3.5 py-2 text-xs sm:text-sm text-foreground focus:border-primary">
                  ...
                  <div className="mt-1.5 flex items-center justify-between text-[11px]">
                    {(projects ?? []).length === 0 ? (
                      <p className="text-muted-foreground">
                        ...
                        <Link to="/projects/new" className="font-bold text-primary hover:underline">
                          Add a project
                        </Link>{" "}
                ...
                <div>
                  <textarea
                    ...
                    className="w-full resize-none rounded-xl border border-border bg-card px-3 py-2 text-xs text-foreground leading-relaxed placeholder:text-muted-foreground focus:border-primary focus:outline-none"
                  />
                  <div className="mt-1.5 flex flex-wrap items-center gap-1.5">
                    <span className="text-[10px] text-muted-foreground mr-0.5">Quick add:</span>
                    {SUBJECT_SUGGESTIONS.map((s) => (
                      <button
                        key={s}
                        type="button"
                        onClick={() => addSuggestion(s)}
                        className="min-h-[22px] rounded-md border border-border bg-black/5 dark:bg-white/5 px-2 py-0.5 text-[10px] font-medium text-foreground hover:bg-black/10 dark:hover:bg-white/10 cursor-pointer active:scale-95 transition-all"
                      >
                        + {s}
                      </button>
                    ))}
            ...
            <div className="apple-glass-card p-3.5 flex-1 flex flex-col justify-between">
              <div className="flex items-center justify-between mb-2">
                <span className="text-[11px] font-bold text-foreground uppercase tracking-wider font-mono">
                  [ 03 · EXAMINER PERSONA ]
                </span>
          ...
          <div className="lg:col-span-5 flex flex-col gap-2.5 lg:gap-3 min-h-0 justify-between">
            ...
            <div className="apple-glass-card p-3.5 space-y-3">
                    ...
                    <button
                      ...
                      className={`min-h-[34px] rounded-xl border px-2 py-1 text-xs font-semibold transition-all cursor-pointer text-center active:scale-95 ${
                        duration === d.minutes
                          ...
                          : "border-border bg-black/5 dark:bg-white/5 text-muted-foreground hover:text-foreground"
                      }`}
                    >
                      {d.label}
                    </button>
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <span className="text-[11px] text-primary font-mono font-bold">{difficulty}</span>
                </div>
                <div className="grid grid-cols-4 gap-1.5">
            ...
            <div className="apple-glass-card p-3.5 flex-1 flex flex-col justify-between">
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-[11px] text-primary font-mono font-bold">{language}</span>
                </div>
              ...
              <div className="mt-3 rounded-xl border border-border bg-black/5 dark:bg-black/40 p-2.5">
                <div className="flex items-center justify-between text-[10px] font-mono">
                  <span className="text-muted-foreground uppercase">DEFENSE CONFIGURATION</span>
                ...
                <div className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-[11px]">
                  <span className="text-foreground font-semibold">{sessionType} Viva</span>
            ...
            {/* Error banner if any */}
            {error && (
              <div className="rounded-xl border border-rose-500/30 bg-rose-500/10 px-3 py-2 text-xs font-mono text-rose-500">
                {error}
              </div>
            )}

            {/* Card 6: Action Footer */}
            <div className="apple-glass-card p-3 flex items-center justify-between gap-2.5">
              <Link
                to="/ai-viva"
                className="apple-glass-btn-secondary min-h-[44px] inline-flex items-center justify-center px-4 text-xs font-semibold no-underline"
              >
                Cancel
              </Link>
              <button
                disabled={mutate.isPending || !canStart}
                onClick={() => void handleStart()}
                className="apple-glass-btn-primary min-h-[44px] flex-1 inline-flex items-center justify-center gap-2 px-5 text-xs sm:text-sm font-bold uppercase tracking-wider cursor-pointer disabled:cursor-not-allowed disabled:opacity-40"
              >
